...
pub fn check_settings(project_dir: &Path, settings_module: &str) -> Vec<Diagnostic> {
    ...
    if let Some(s) = settings.get("DEBUG") {
        if matches!(s.value, SettingValue::Bool(true)) {
            diagnostics.push(make_diag("DJ012", "DEBUG = True in final settings. Use an environment variable.", &s.file, s.line));
        }
    }
    ...
    if let Some(s) = settings.get("SECRET_KEY") {
        if matches!(s.value, SettingValue::HardcodedString) {
            diagnostics.push(make_diag("DJ016", "SECRET_KEY is hardcoded. Use os.environ['SECRET_KEY'].", &s.file, s.line));
        }
    }
    ...
    for (name, purpose) in security_settings {
        if !settings.contains_key(name) {
            diagnostics.push(make_diag("DJ013", &format!("{name} is not set — {purpose}. See Django deployment checklist."), &rel_path, settings_last_line));
        }
    }
...
fn collect_settings(file: &Path, project_dir: &Path, settings: &mut HashMap<String, ResolvedSetting>, visited: &mut Vec<PathBuf>) {
        ...
        match stmt {
            Stmt::ImportFrom(import) => {
                if let Some(module_name) = &import.module {
                    ...
                    if imports_star {
                        if let Some(imported_file) = resolve_import(file, module_name.as_str(), Some(import.level), project_dir) {
                ...
                for target in &assign.targets {
                    if let Expr::Name(n) = target {
                        ...
                        if name.chars().all(|c| c.is_uppercase() || c == '_') && name.len() > 1 {
                            ...
                            settings.insert(name.to_string(), ResolvedSetting { value, file: filename.clone(), line });
                        }
                    }
                }
            }
...
fn resolve_import(current_file: &Path, module_name: &str, level: Option<u32>, project_dir: &Path) -> Option<PathBuf> {
    let level = level.unwrap_or(0);
    if level > 0 {
        let mut base = current_file.parent()?.to_path_buf();
        for _ in 1..level { base = base.parent()?.to_path_buf(); }
